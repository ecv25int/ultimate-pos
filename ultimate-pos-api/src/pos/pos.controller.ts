...
export class PosController {
  ...
  @Get('products/scan')
  @ApiOperation({
    summary: 'Scan barcode / look up by exact SKU',
    description:
      'Returns a single product matching the exact barcode/SKU, including current stock.',
  })
  ...
  @ApiOperation({
    ...
    description: 'Fast product search used by the POS terminal. Results cached 1 min.',
  })
  ...
  @ApiOperation({
    ...
    description: 'Creates a finalized sale from the POS terminal. Decrements stock immediately.',
  })
  ...
  processTransaction(@Request() req: any, @Body() dto: CreateSaleDto) {
    return this.posService.processTransaction(req.user.businessId, req.user.id, dto);
  }
  ...
  @ApiOperation({
    ...
    description: 'Sends the ESC/POS pulse command to the configured cash drawer via TCP or serial.',
  })
  @ApiResponse({
    status: 201,
    description: 'Command sent (or silently skipped when not configured).',
  })
  ...
  @Get('recent')
  @ApiOperation({
    summary: 'Recent POS transactions',
    description: 'Returns the last 50 POS sales for the current business.',
  })
